...
Alternatively, you can use the `dropbox` command in place of the `process` command to automatically upload the compressed data files and insert the shareable links in place of instances of `[DATA_DOWNLOAD_LINK]` in the documents. For this to work you need to have the Dropbox Python SDK installed and a Dropbox access token accessible via the `DROPBOX_TOKEN_ENV_VAR` environment variable.
