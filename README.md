...
* `data` - `.zip` files, one for each document for which content was provided in the `data` or `data_not_tracked` folders.
...
3. Name the app (e.g., `document-builder`)
4. Enable the following OAuth scopes:
...
5. Set the OAuth 2 redirect URI to:
...
```bash
python authorize-dropbox.py
```

Follow the prompts:

- Open the provided Dropbox authorization URL in your browser
- Click "Allow"
...
```bash
# Dropbox config for document-builder
# your_app_key and your_app_secret can be found in the Dropbox App Console
export DOCUMENT_BUILDER_DROPBOX_APP_NAME="document-builder"
export DOCUMENT_BUILDER_DROPBOX_APP_KEY="your_app_key"
export DOCUMENT_BUILDER_DROPBOX_APP_SECRET="your_app_secret"
export DOCUMENT_BUILDER_DROPBOX_TOKEN_FILE="$HOME/.document-builder-secrets.json"
```
...
- If your token ever becomes invalid, just re-run:
...
The data sharing functionality works with assignment mode. Any instances of `[DATA_DOWNLOAD_LINK]` in the Markdown documents will be replaced with the sharable links for the `.zip` files in the `data_to_share_links` folder.
